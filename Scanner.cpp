...
void Scanner::Init() {
	...
	keywords.set(L"return", 19);
	keywords.set(L"if", 20);
	keywords.set(L"then", 21);
	keywords.set(L"endif", 22);
	keywords.set(L"while", 23);
	keywords.set(L"do", 24);
	keywords.set(L"end", 25);
	keywords.set(L"function", 26);
	keywords.set(L"procedure", 27);
	keywords.set(L"define", 28);
	keywords.set(L"variable", 29);
	keywords.set(L"parameter", 30);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 1:
			case_1:
			if (ch <= L'!' || (ch >= L'#' && ch <= 65535)) {AddCh(); goto case_1;}
		...
		case 3:
			...
			if (ch == L'$' || (ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_3;}
